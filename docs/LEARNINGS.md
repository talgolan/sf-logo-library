...
export const ALL_FOO: readonly Foo[] = ["a", "b", "c"];

// Exhaustiveness: fails to compile if a union member is missing.
const _ALL_FOO_EXHAUSTIVE: { readonly [K in Foo]: true } = {
  a: true, b: true, c: true,
};
void _ALL_FOO_EXHAUSTIVE;
```

The mapped type forces a property per union member (missing key = TS2741). The `void` statement silences `no-unused-vars` since the variable exists only for its type-checking side effect.
— Adopted in phase-3E for `AdvisoryCode` (commit `add2fc7`).
